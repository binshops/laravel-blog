...
abstract class BaseHessamCMSPostRequest extends BaseRequest
{
    ...
    protected function baseBlogPostRules()
    {
        ...
        if (config('hessamcms.use_custom_view_files')) {
        ...
        foreach ((array)config('hessamcms.image_sizes') as $size => $image_detail) {
            if ($image_detail['enabled'] && config("hessamcms.image_upload_enabled")) {
